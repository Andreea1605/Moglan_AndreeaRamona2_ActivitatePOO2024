...
public:
	...
	void scriereFisierBinar(fstream& fisier) {
		...
		for (int i = 0; i < nrRoti; i++) {
			...
			fisier.write((char*)(producatori[i].c_str()), lungime * sizeof(char));//transformarein sir de caractere
		}
		int lungime2 = marca.length();
		fisier.write((char*)&lungime2, sizeof(lungime2));
		fisier.write((char*)(marca.c_str()), lungime2 * sizeof(char));
	...
	void citesteFisierbinar(fstream& fisier) {
		...
		for (int i = 0; i < nrRoti; i++) {
			...
			char buffer[50];
			fisier.read((char*)(&buffer), lungime * sizeof(char));//transformare in sir de caractere
			this->producatori[i] = string(buffer);
		}
		int lungime2 = 0;
		fisier.read((char*)&lungime2, sizeof(lungime2));
		fisier.read((char*)(marca.c_str()), lungime2 * sizeof(char));
...
int main() {
	...
	int nrRoti = 4;
	string* producatori = new string[nrRoti];
	producatori[0] = "Michelin";
	producatori[1] = "Pirelli";
	producatori[2] = "Hankook";
	producatori[3] = "Bridgestone";
	Masina m(4, "Dacia", 5000.0f, 2020);
	m.setNrRoti(nrRoti, producatori);
	fisier << m;
	Masina m2(4, "Volvo", 55000.0f, 2021);
	fisier << m2;
	fisier.close();*/
	...
	masina.setNrRoti(nrRoti, producatori);
