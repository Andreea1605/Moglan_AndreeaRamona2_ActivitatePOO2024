...
public:
	...
	friend istream& operator>>(istream& input, Masina& m) { // nu se poate modifica daca se transmite prin valoare si se adauga &
		cout << "Marca:";
		input >> m.marca;
		cout << "Nr roti:";
		input >> m.nrRoti;
...
int main() {
	...
	int nrRoti = (int)m2;
	cout << nrRoti << endl;
	cout << (int)m2 << endl;//cast explixit
	try {
		cout << m2[1] << endl;
		/*cout<<m2[-4]<<endl;*/
		//cout<<"ceva" // nu se mai afiseaza, se intrerupe blocul de try
		m2[1] = "Hankook";
		cout << m2[1] << endl;
	}
	catch (int ex) {
