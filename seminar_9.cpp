...
public:
	...
	Tir(const char* tara, float tonaj) : Masina(10, "Volvo", 100000, 2020) {
	...
	friend istream& operator>>(istream& input, Tir& tir) {
		...
		tir.tara = new char[strlen(tir.tara) + 1];
...
int main() {
	...
	Tir t2;
	int numarRoti = 4;
	string* producatori = new string[numarRoti];
	for (int i = 0; i < numarRoti; i++) {
		producatori[i] = "Michelin";
	}
	Tir t3("Romania", 15.2f, "Dac", numarRoti, 50000.0f, producatori, 2019);
	Tir t4 = t3;
	Tir t5;
	t5 = t3;
	cout << t3;
	...
	cout << endl << "Media de tone pe roata este de:" << t5.calculeazaMediaTonaj() << endl;
